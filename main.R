...
make_geojson <- function (stations_ranking, segments_ranking, filename = NULL) {
    ...
    min_alpha <- 10
    exp_base <- (100 - min_alpha) ^ (1 / (max_segment_delay - min_segment_delay))
            ...
            unname(lapply(lapply(split(segments_ranking, seq_along(segments_ranking[, 1])), as.list), function (segment) {
                return(list(
                    ...
                    properties = do.call(c, list(
                        ...
                        "alpha" = (min_alpha + exp_base ^ (segment$average_delay - min_segment_delay)) / 100,
                        "stroke" = substring(col2hcl("red", alpha = round((min_alpha + exp_base ^ (segment$average_delay - min_segment_delay)) / 100, 1)), 1, 7),
                        "stroke-opacity" = 1.0,
                        "stroke-width" = 2.0
                    ))
                ))              
